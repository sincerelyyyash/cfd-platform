...
export default function AssetButton({ imageURL, text, isActive, onClickAction }: ButtonProps) {
  return (
    <button
    ...
    >
      <Image src={imageURL} alt={text} width={24} height={24} className="rounded-full" />
      <span>{text}</span>
    </button>
  );
}

...
export default function TradingModal() {
	...
	return (
		<div className="flex h-screen flex-col border-l border-neutral-900 bg-black p-4">
			...
			{/* Segmented Buy/Sell and tabs */}
			<div className="mt-4 w-full rounded-xl border border-neutral-900/70 bg-black/60 p-3">
				<div className="flex w-full rounded-lg bg-neutral-900/60 p-1">
					<button
						className={
							"flex-1 rounded-md px-3 py-2 text-sm font-semibold transition-colors " +
							(side === "buy" ? "bg-emerald-500/15 text-emerald-300" : "text-zinc-400 hover:text-zinc-200")
						}
						onClick={() => setSide("buy")}
					...
					<button
						className={
							"flex-1 rounded-md px-3 py-2 text-sm font-semibold transition-colors " +
							(side === "sell" ? "bg-rose-500/15 text-rose-300" : "text-zinc-400 hover:text-zinc-200")
						}
				...
				{/* Market-only UI (no explicit label text) */}
				...
				<div className="mt-4">
					...
					<div className="mt-2 flex h-12 w-full items-center justify-between rounded-lg border border-neutral-800 bg-black px-3">
						<input
						...
						/>
						<span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-400">$</span>
					</div>
				</div>
				...
				<div className="mt-4">
					...
					<div className="mt-2 flex h-12 w-full items-center justify-between rounded-lg border border-neutral-800 bg-black">
						<input
							id="volume"
							inputMode="decimal"
							type="text"
						...
						onChange={(e) => {
							...
							placeholder="0.1"
							className="w-full bg-transparent px-3 text-sm text-zinc-200 outline-none placeholder:text-zinc-500"
				...
				<div className="mt-4">
					...
					<div className="mt-2 flex h-12 w-full items-center justify-between rounded-lg border border-neutral-800 bg-black px-3">
						<div className="text-sm text-zinc-200 tabular-nums">{orderValue || 0}</div>
						<span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-400">$</span>
					</div>
				</div>
			</div>
			...
			<div className="mt-3 flex items-center justify-between text-xs text-zinc-400">
				...
				<span className="rounded-md border border-neutral-800 bg-neutral-900 px-2 py-0.5 text-zinc-200" aria-live="polite">{selectedLeverage}</span>
			</div>
	...
	<div className="w-full pt-2">
		<button
			...
			className="flex w-full items-center justify-between rounded-lg border border-neutral-800 bg-black px-3 py-2 text-left text-sm text-zinc-300 hover:bg-neutral-950"
		>
			<span className="flex items-center gap-2">
				<span className="text-zinc-200">Advanced (optional)</span>
		...
		{showRiskFields && (
			<div className="mt-3 grid grid-cols-1 gap-3">
				<div className="space-y-2">
					...
					<div className="flex h-12 w-full items-center justify-end rounded-lg border border-neutral-800 bg-black">
						<input
							id="take-profit"
							inputMode="decimal"
							type="text"
			...
			<div className="mt-4 flex gap-3">
				...
				>
					{side === "buy" ? "Buy" : "Sell"} • {displayedPrice ? displayedPrice.toFixed(priceInfo.decimals) : "0"}
				</Button>
			</div>
		</div>
	);
